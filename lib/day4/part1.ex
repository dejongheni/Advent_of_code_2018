      ...
      |> Enum.map(fn {guard_id, actions} ->
        ...
        map_of_days =
          if map_of_days == nil do
            ...
            total_asleep =
              Map.values(map_of_days)
              |> Enum.reduce(0, fn val, acc -> acc + length(val.asleep_minutes) end)
            Map.update(map_of_days, :total_asleep, total_asleep, fn _val -> total_asleep end)
          end
        {guard_id, map_of_days}
      end)
      |> Enum.into(%{})

    {laziest_guard, _time_asleep} =
      Map.to_list(map_of_guards)
      |> Enum.map(fn {id, map_of_times} -> {id, map_of_times.total_asleep} end)
      |> Enum.max_by(fn val -> elem(val, 1) end)
    {minute_most_asleep, _number_of_times} =
      Map.get(map_of_guards, laziest_guard)
      |> Map.delete(:total_asleep)
      |> Map.to_list
